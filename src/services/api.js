...
// Get API URL with proper handling for empty string
const getApiUrl = (endpoint) => {
  if (!API_URL) {
    console.warn('API_URL not configured. Backend calls will fail. Set VITE_API_URL environment variable.')
    return endpoint
  }
...
export async function createScreening(screeningData) {
  try {
    const response = await fetch(getApiUrl('/api/v1/screenings'), {
      method: 'POST',
      headers: {
...
export async function getScreening(screeningId) {
  try {
    const response = await fetch(getApiUrl(`/api/v1/screenings/${screeningId}`))
    if (!response.ok) {
...
export async function getScreeningStatus(screeningId) {
  try {
    const response = await fetch(getApiUrl(`/api/v1/screenings/${screeningId}/status`))
    if (!response.ok) {
